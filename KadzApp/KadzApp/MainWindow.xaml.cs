...
namespace KadzApp
{
	...
	public partial class MainWindow : Window
	{
		...
		private void InitComboBoxes()
		{
			...
			steel_.SelectedIndexWeightUnit = 1;
			//Density units
		...
		private void CbWeight_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			if (cbSteelWeight.SelectedIndex >= 0)
			{
				...
				steel_.SelectedIndexWeightUnit = cbSteelWeight.SelectedIndex;
				RecalcSteel();
			}
		}
		...
		private void CbDensity_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			if (cbSteelDensity.SelectedIndex >= 0)
			{
				...
				steel_.SelectedIndexDensityUnit = cbSteelDensity.SelectedIndex;
				RecalcSteel();
			}
		}
		...
		private void CbSize_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			if (cbSteelSize.SelectedIndex >= 0)
			{
				...
				steel_.SelectedIndexSizeUnit = cbSteelSize.SelectedIndex;
				RecalcSteel();
			}
		}
		...
		private void BtnReadVat_Click(object sender, RoutedEventArgs e)
		{
			...
			if (result == true)
			{
				...
				{
					string fileName = dlg.FileName;
					var saveDto = JsonConvert.DeserializeObject<SaveDto>(File.ReadAllText(fileName));

					vat_ = saveDto.Vat;
					argon_ = saveDto.Argon;
					steel_ = saveDto.Steel;

					txbSteelWeight.Text = $"{steel_.Weight}";
					cbSteelWeight.SelectedIndex = steel_.SelectedIndexWeightUnit;

					txbSteelDensity.Text = $"{steel_.Density}";
					cbSteelDensity.SelectedIndex = steel_.SelectedIndexDensityUnit;

					txbSteelSize.Text = $"{steel_.Size}";
					cbSteelSize.SelectedIndex = steel_.SelectedIndexSizeUnit;
					...
					if (argon_.Algorithms == Algorithms.Density)
					{
						Mole.IsChecked = false;
						Density.IsChecked = true;
					}
					else
					{
						Mole.IsChecked = true;
						Density.IsChecked = false;
					}
					txbArgonFlowIndu.Text = $"{argon_.IndustryFlow}";
					RecalcArgon();
				}
				catch (Exception ex)
				{
		...
		{
			argon_.Algorithms = Algorithms.Mole;
			RecalcArgon();
		}

		private void RadioButton_Checked_1(object sender, RoutedEventArgs e)
		{
			argon_.Algorithms = Algorithms.Density;
			RecalcArgon();
		}

		private void TxbVatScale_1_TextChanged(object sender, TextChangedEventArgs e)
		{
			if (int.TryParse(txbVatScale_1.Text, out int left) && int.TryParse(txbVatScale_2.Text, out int right))
			{
				vat_.VatscaleRight = right;
		...
		private void TxbVatScale_2_TextChanged(object sender, TextChangedEventArgs e)
		{
			if (int.TryParse(txbVatScale_1.Text, out int left) && int.TryParse(txbVatScale_2.Text, out int right))
			{
				vat_.VatscaleRight = right;
				vat_.VatscaleLeft = left;
				double scale = Convert.ToDouble(left) / Convert.ToDouble(right);
				argon_.SetScale(scale);
				RecalcSmallVat();
				RecalcArgon();
			}
		}
